#ifndef Z_SHADOW_MAPPING_H
#define Z_SHADOW_MAPPING_H
...
float ZRP_ShadowMap01(float4 worldPos, sampler2D _shadowtex, float4x4 _shadowVpMatrix)
{
...
float ZRP_PCF3x3(float4 worldPos, sampler2D _shadowtex, float4x4 _shadowVpMatrix, float shadowMapResolution, float bias)
{
    float4 shadowNdc = mul(_shadowVpMatrix, worldPos);
    shadowNdc /= shadowNdc.w;
    float2 uv = shadowNdc.xy * 0.5 + 0.5;
...
float2 ZRP_RotateVec2(float2 v, float angle)
{
    float s = sin(angle);
    float c = cos(angle);
...
float2 ZRP_AverageBlockerDepth(float4 shadowNdc, sampler2D _shadowtex, float d_shadingPoint, float searchWidth, float rotateAngle)
{
    float2 uv = shadowNdc.xy * 0.5 + 0.5;
    float step = 3.0;
    float d_average = 0.0;
    ...
    */
    ...
    {
        float2 unitOffset = ZRP_RotateVec2(poissonDisk[i], rotateAngle);
        float2 offset = unitOffset * searchWidth;
        float2 uvo = uv + offset;
    ...
    float pcssSearchRadius, float pcssFilterRadius)
{
    ...
    float2 blocker = ZRP_AverageBlockerDepth(shadowNdc, _shadowtex, d_shadingPoint, searchWidth, rotateAngle);
    ...
    */
    ...
    for(int i=0; i<N_SAMPLE; i++)
    {
        ...
        offset = ZRP_RotateVec2(offset, rotateAngle);
        float2 uvo = uv + offset * radius;
        //if(uvo.x<0 || uvo.x>1 || uvo.y<0 || uvo.y>1) continue;
