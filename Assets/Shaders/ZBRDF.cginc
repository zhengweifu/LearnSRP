...
// D
float ZRP_Trowbridge_Reitz_GGX(float NdotH, float a)
{
    float a2     = a * a;
...
// F
float3 ZRP_SchlickFresnel(float HdotV, float3 F0)
{
...
// G
float ZRP_SchlickGGX(float NdotV, float k)
{
    float nom   = NdotV;
...
float3 ZRP_FresnelSchlickRoughness(float NdotV, float3 f0, float roughness)
{
...
// 直接光照
float3 ZRP_PBR(float3 N, float3 V, float3 L, float3 albedo, float3 radiance, float roughness, float metallic)
{
    ...
    float  D = ZRP_Trowbridge_Reitz_GGX(NdotH, alpha);
    float3 F = ZRP_SchlickFresnel(HdotV, F0);
    float  G = ZRP_SchlickGGX(NdotV, k) * ZRP_SchlickGGX(NdotL, k);
    ...
    samplerCUBE _diffuseIBL, samplerCUBE _specularIBL, sampler2D _brdfLut)
{
    ...
    float3 F = ZRP_FresnelSchlickRoughness(HdotV, F0, roughness);
    float3 k_s = F;
    float3 k_d = (1.0 - k_s) * (1.0 - metallic);
